...
from .access import MyNetworkAccessManager, WebEngineUrlSchemeHandler, _load_static_data
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self._url_scheme_handler = WebEngineUrlSchemeHandler(self)
        ...
        # Install URL scheme handlers on the WebEngine profile
        profile = self._ui.webView.page().profile()
        profile.installUrlSchemeHandler(b"dict", self._url_scheme_handler)
        profile.installUrlSchemeHandler(b"static", self._url_scheme_handler)
        profile.installUrlSchemeHandler(b"search", self._url_scheme_handler)
        # Note: audio:// URLs are handled through navigation interception, not URL scheme handler
    ...
    def _updateTitle(self, title):
        title = title.strip()
        if title:
            self.setWindowTitle(f"{title} - {QApplication.applicationName()}")
        else:
    ...
    def _incremental_search(self, key):
        ...
        except (OSError, incremental.IndexError):
            return None
    ...
    def _getAudioData(self, path, callback):
        ...
        if archive in ("us_hwd_pron", "gb_hwd_pron", "exa_pron", "sfx", "sound"):
            try:
                ...
                # Get the audio data directly from LDOCE5
    ...
    def _onUrlChanged(self, url):
        # WebEngine doesn't store/restore navigation history items' metaData like WebKit
        history = self._ui.webView.history()
        items = []
        for i in range(history.count()):
            items.append(history.itemAt(i))
        curidx = history.currentItemIndex()

        def update_navmenu(menu, items, curidx, back):
            def make_goto(idx):
                def f():
                    history.goToItem(items[idx])

                return f

            menu.clear()
            if back:
                item_range = list(range(curidx - 1, -1, -1))[:10]
            else:
                item_range = list(range(curidx + 1, len(items)))[:10]

            for idx in item_range:
                hist_item = items[idx]
                title = hist_item.title()
                if not title:
                    title = hist_item.url().toString()
                title = ellipsis(title, 60)
                action = menu.addAction(title, make_goto(idx))
        ...
        # restore search phrase - WebEngine compatibility
        hist_item = history.currentItem()
        curr_query = self._ui.lineEditSearch.text()

        # WebEngine doesn't support userData() like WebKit did
        # We'll handle this gracefully by skipping the history-based query restoration
        try:
            # Try to access userData (WebKit compatibility)
            ...
            if hist_query:
                ...
                hist_item.setUserData(curr_query)
        except AttributeError:
            # WebEngine doesn't have userData() - this is expected
            # Just update the index without history-based query restoration
            self._timerUpdateIndex.start(0)
    ...
    def _onNavForward(self):
        self._ui.webView.forward()

    def _onNavBack(self):
        self._ui.webView.back()

    def _onNavActionChanged(self):
        # WebEngine doesn't have direct action access like WebKit
        # We'll just enable/disable based on history
        webPage = self._ui.webView.page()
        ui = self._ui
        # For now, we'll assume these are always enabled
        # A proper implementation would need to track history state
        ui.toolButtonNavForward.setEnabled(True)
        ui.toolButtonNavBack.setEnabled(True)
    ...
    def findText(self, text):
        ...
        # WebEngine has different find API
        if text:
            self._ui.webView.findText(text)
        else:
            self._ui.webView.findText("", QWebEnginePage.FindFlags())

        # WebEngine doesn't return found status directly
        # We'll assume it's found if there's text
        found = bool(text)
    ...
    def findNext(self):
        text = self._ui.lineEditFind.text()
        if text:
            self._ui.webView.findText(text)
    ...
    def findPrev(self):
        text = self._ui.lineEditFind.text()
        if text:
            self._ui.webView.findText(text, QWebEnginePage.FindBackward)
    ...
    def printPreview(self):
        ...
        # WebEngine print API is different
        def print_page(printer):
            ui.webView.page().print(printer, lambda success: None)

        dialog.paintRequested.connect(print_page)
        dialog.exec_()
    ...
    def _setup_ui(self):
        ...
        # WebEngine doesn't have SelectAll action accessible like WebKit
    ...
    def _updateNetworkAccessManager(self, fulltext_hp, fulltext_de):
        # WebEngine doesn't support setNetworkAccessManager like WebKit
        # Custom network access is handled differently with URL scheme handlers
        nwaccess = MyNetworkAccessManager(self, fulltext_hp, fulltext_de)

        # Update URL scheme handler with new searchers
        self._url_scheme_handler.update_searchers(fulltext_hp, fulltext_de)

        # Keep the network access manager for compatibility
    ...
    def _fts_hwdphr(self):
        ...
        if obj is None:
            ...
            except (OSError, fulltext.IndexError):
                pass
