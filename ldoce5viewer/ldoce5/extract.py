...
def _get_incorrect_inflections(
    ...
    def handle_adjective():
        ...
        if (
            num_syllable >= 3
            or num_syllable >= 2
            and not (endswith("y") or endswith("le") or endswith("er"))
        ):
            return make_comparative()
...
def get_entry_items(entry_data):
    ...
    def make_hwd_label():
        ...
        if homnum is not None:
            baselabel += f"<s>{escape(_get_text(homnum))}</s>"

        if is_freq:
            hwdlabel = f"<f>{baselabel}</f>"
        else:
            hwdlabel = f"<n>{baselabel}</n>"
    ...
    def get_hwd_variants():
        ...
        for inflx in hwd.iterfind("INFLX"):
            ...
            if inflxplain in incorrect:
                continue
            inflxlabel = f"<h><v>{escape(inflxplain)}</v> &rarr; {hwdlabel}</h>"
        ...
        for lexvar in variants:
            ...
            for v_plain in v_plains:
                if v_plain in incorrect:
                    continue
                v_label = f"<h><v>{escape(v_plain)}</v> &rarr; {hwdlabel}</h>"
    ...
    def get_phrvb(phrvb):
        ...
        label = f"<h><pv>{escape(plain)}</pv> <p>phrasal verb</p></h>"
    ...
    def get_runon(runon):
        ...
        for inflx in deriv.iterfind("INFLX"):
            ...
            if inflplain in incorrect:
                continue
            infllabel = f"<h><v>{escape(inflplain)}<v> &rarr; {label}</h>"
    ...
    def get_lexunit(elem):
        ...
        label = f"<l><o>{escape(plain)}</o> ({hwdlabel})</l>"
    ...
    def get_simple(elem):
        ...
        path = "/fs/{0}#{1}".format(root_id, shorten_id(elem.get("id")))
        plain = _get_text(elem)
        label = f"<c><o>{plain}</o> ({hwdlabel})</c>"
    ...
    def get_collocate(elem):
        ...
        for var in variants:
            ...
            v_label = f"<c><o>{v_plain}</o> ({hwdlabel})</c>"
    ...
    def get_example(elem):
        ...
        if collo:
            ...
            colabel = f"<c><o>{colabel}</o> ({hwdlabel})</c>"
    ...
    def get_sense(elem):
        ...
        for var in variants:
            ...
            v_label = f"<l><o>{escape(v_plain)}</o> ({hwdlabel})</l>"
